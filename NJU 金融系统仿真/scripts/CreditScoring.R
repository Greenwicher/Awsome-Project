...
## get data from the RSADBE package
data(GC)
data<-GC
data$purpose<-as.integer(data$purpose)
data$property<-as.integer(data$property)
...
## logistic regression
fit1<-glm(good_bad~., data=train, family=binomial(link="logit"))
summary(fit1)
write.csv(coef(summary(fit1)),  'logistic.csv')
test$score1<-predict(fit1, type='response', test)
pred1<-prediction(test$score1, test$good_bad)
...
fit2<-rpart(good_bad~.,data=train) 
...
## random forests
fit3<-randomForest(good_bad~., data=train, importance=TRUE, proximity=TRUE, ntree=500, keep.forest=TRUE)
...
pred3<-prediction(test$score3,test$good_bad) 
...
legend(0.8,0.6,c('LR','CT','RF'),col=c('red','green','blue'),lwd=3) 
...
save.image('CreditScoring.RData')
